...
public class ProcessPaymentCommandHandlerTests
{
    ...
    {
        var contextMock = new Mock<IPaymentDbContext>();
        var publishMock = new Mock<IPublishEndpoint>();

        contextMock.Setup(x => x.Payment).ReturnsDbSet(new List<PaymentRecord>());
        
        var paymentList = new List<PaymentRecord>();
        contextMock.Setup(x => x.Payment.Add(It.IsAny<PaymentRecord>())).Callback<PaymentRecord>(p => paymentList.Add(p));
