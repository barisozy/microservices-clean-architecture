...
public sealed class SetStockCommandHandler(IInventoryWriteRepository context, IStockReadRepository stockReadRepository) : IRequestHandler<SetStockCommand, int>
{
    public async Task<int> Handle(SetStockCommand request, CancellationToken cancellationToken)
    {
        ...
        if (stock is null) { stock = new Stock(request.Sku, request.Quantity); context.Add(stock); }
