...
public class ProcessPaymentCommandHandler(IPaymentWriteRepository context, IPublishEndpoint publishEndpoint, IPaymentReadRepository paymentReadRepository) : IRequestHandler<ProcessPaymentCommand, Guid>
{
    public async Task<Guid> Handle(ProcessPaymentCommand request, CancellationToken cancellationToken)
    {
        var existingPayment = await context.FindByIdempotencyKeyAsync(request.IdempotencyKey, cancellationToken);
